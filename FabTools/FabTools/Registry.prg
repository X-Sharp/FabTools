...
CLASS FabRegistry	INHERIT	FabConfigFile
	...
	PROTECT METHOD _ExportEntryToString( cSection AS STRING, cEntry AS STRING )
		...
		IF ( SELF:LastError == ERROR_SUCCESS )
			...
			ENDIF
			//
			IF ( dwType != (DWORD)REG_SZ )  .AND.	;
				( dwType != (DWORD)REG_DWORD )
				// Complex Type
				// Build Entry String
				IF ( dwType != (DWORD) REG_BINARY )
	...
	METHOD GetInt( cSection, cEntry, nDefault )
		...
		IF ( SELF:LastError == ERROR_SUCCESS )
			//
			IF ( dwType == (DWORD)REG_DWORD_BIG_ENDIAN )  .OR. ( dwType == (DWORD)REG_DWORD )
				// Ok read data
	...
	METHOD GetSection( cSection )
		...
		IF ( SELF:LastError == ERROR_SUCCESS  )
			...
			FOR dwIndex := 1 TO ALen( aEntries )
				...
				//
				IF 	( dwType == (DWORD)REG_EXPAND_SZ )      .OR. ;
					( dwType == (DWORD)REG_BINARY )	     .OR. ;
					( dwType == (DWORD)REG_LINK )           .OR. ;
					( dwType == (DWORD)REG_NONE )           .OR. ;
					( dwType == (DWORD)REG_RESOURCE_LIST )  .OR. ;
					( dwType == (DWORD)REG_SZ )             .OR. ;
					( dwType == (DWORD)REG_MULTI_SZ )
					// Get Data as string
					sData := SELF:GetString( cSection, aEntries[ dwIndex ] )
					//
					AAdd( aData, { aEntries[ dwIndex ], sData, dwType } )
				ELSEIF 	( dwType == (DWORD)REG_DWORD )          .OR. ;
					( dwType == (DWORD)REG_DWORD_BIG_ENDIAN )
					// Get Data as LONGINT
