...
//j METHOD:FabTwain:ModalAcquire
	...
	//
		PROTECT ptrDSM_Entry	AS	__FabDSM_Entry PTR
	...
	// Change the state based on the requested operation.
			IF ( dg == DG_CONTROL )
				...
				ELSEIF ( dat == DAT_IDENTITY )
					IF ( msg == MSG_CLOSEDS )
						IF lOk
							SELF:State := SOURCE_MANAGER_OPEN
						ENDIF
					ENDIF
				ENDIF
			ELSEIF ( dg == DG_IMAGE )
...
//the real DS Call
	...
	// Data Source Call
			siResult := PCALL( SELF:ptrDSM_Entry, SELF:pAppID, SELF:pSourceId, dg, dat, msg, pd )
			RETURN siResult
...
// Remove all pending messages for the Twain DataSource UI
	...
	//
			WHILE ( ( SELF:dwCurState >= SOURCE_ENABLED )  .AND. PeekMessage( @msg, NULL, 0, 0, PM_REMOVE) )
...
// Load the Source Manager DLL
	...
	//
	// SM already loaded ?
			IF ( SELF:dwCurState >= SOURCE_MANAGER_LOADED )
				RETURN TRUE
			ENDIF
	//
	...
	//
			...
			IF File( cDir )
				SELF:hSMLib := _VOLoadLibrary( cDir )
			ELSE
				SELF:__Trace( "Error Loading Library." )
				SELF:hSMLib := NULL_PTR
			ENDIF
	//
			IF ( SELF:hSMLib != NULL_PTR )
				SELF:ptrDSM_Entry := GetProcAddress( hSMLib, String2Psz( DSM_ENTRYPOINT ) )
				IF ( SELF:ptrDSM_Entry != NULL_PTR )
					SELF:State := SOURCE_MANAGER_LOADED
				ELSE
					SELF:__Trace( "GetProcAddress() Failed." )
					_VOFreeLibrary( SELF:hSMLib )
					SELF:hSMLib := NULL_PTR
				ENDIF
			ELSE			
			...
			ENDIF
	//
			IF ( SELF:State != SOURCE_MANAGER_LOADED )
				SELF:__Trace( "LoadSourceManager() Failed." )
			ENDIF
			RETURN ( SELF:State >= SOURCE_MANAGER_LOADED )
...
// Call the Source Manager with a triplet. See the Twain Spec for more info
			LOCAL lOk	AS	LOGIC
	//
			ASSERT ( SELF:State >= SOURCE_MANAGER_LOADED )
	//
...
// Unload the Source Manager DLL
	//
			SELF:siError := TWRC_SUCCESS
	//
			IF ( SELF:State == SOURCE_MANAGER_LOADED )
				IF ( SELF:hSMLib != NULL_PTR )
					_VOFreeLibrary(hSMLib)
					SELF:hSMLib := NULL_PTR
				ENDIF
				SELF:ptrDSM_Entry := NULL_PTR
				SELF:State := PRE_SESSION
			ENDIF
			RETURN ( SELF:State == PRE_SESSION )
...
//r A logical value indicating the success of the operation.
	...
	//
			dwState := SELF:State
	//
			IF ( SELF:State >= SOURCE_MANAGER_OPEN )  .OR. ( SELF:OpenSourceManager() )
				IF ( SELF:State >= SOURCE_OPEN )  .OR. ( SELF:OpenSource( SELF:SourceName )  .AND. SELF:OnTwainSourceOpen() )
					IF ( SELF:State >= SOURCE_ENABLED )  .OR. ( SELF:EnableSource() )
						lBeginOk := TRUE
					ELSE
...
//d Set the current brightness value
			LOCAL lOk	AS	LOGIC
	//
			lOk := SELF:SetCapFix32(ICAP_BRIGHTNESS, rBrightness )
...
//r A logical value indicating the success of the operation.
	//
			SELF:siError := TWRC_SUCCESS
	//
			IF ( SELF:dwCurState == SOURCE_ENABLED )
				SELF:DisableSource()
			ENDIF
			IF ( SELF:dwCurState == SOURCE_OPEN )
				SELF:__DS( DG_CONTROL, DAT_IDENTITY, MSG_CLOSEDS, SELF:pSourceId )
				IF ( SELF:dwCurState == SOURCE_OPEN )
					SELF:__Trace("TWAIN: CLOSEDS failed.")
				ENDIF
			ENDIF
			RETURN ( SELF:dwCurState < SOURCE_OPEN )
...
//r A logical value indicating the success of the operation
	...
	// close source if open
			SELF:CloseSource()	
	//
			IF ( SELF:State >= SOURCE_MANAGER_OPEN )
		//
				...
				ENDIF
			ENDIF
			RETURN ( SELF:State < SOURCE_MANAGER_OPEN )
...
//d Set the current contrast value
			LOCAL lOk	AS	LOGIC
	//
			lOk := SELF:SetCapFix32(ICAP_CONTRAST, rContrast )
	...
	//
		...
		//
			...
			// Ok !! Bring back focus to the owner
				...
				ENDIF
			ENDIF
			RETURN ( SELF:dwCurState < SOURCE_ENABLED )
...
//r A Logical value indicating the success of the operation
	...
	//
			WHILE ( SELF:dwCurState > nNewState )
				DO CASE
					CASE ( SELF:dwCurState == TRANSFERRING )
						lOk := SELF:EndXfer()
					CASE ( SELF:dwCurState == TRANSFER_READY )
						lOk := SELF:CancelXfers()
					CASE ( SELF:dwCurState == SOURCE_ENABLED )
						lOk := SELF:DisableSource()
					CASE ( SELF:dwCurState == SOURCE_OPEN )
						lOk := SELF:CloseSource()
					CASE ( SELF:dwCurState == SOURCE_MANAGER_OPEN )
						lOk := SELF:CloseSourceManager()
					CASE ( SELF:dwCurState == SOURCE_MANAGER_LOADED )
...
//r A Logical value indicating the success of the operation
	...
	//
			SELF:pUI.ShowUI := IIF( SELF:lShowUI, 1, 0 )
...
//r A Logical value indicating the success of the operation
	//
			IF ( SELF:dwCurState == TRANSFERRING)
			...
			ENDIF
			RETURN ( SELF:dwCurState < TRANSFERRING )
...
//r An array with all avaible source names
	...
	//
		...
		// Data Source Call
				...
				ENDIF
		//
				WHILE ( SELF:ErrorCode == TWRC_SUCCESS )
...
//a This object can be a Window or a Control, and provide some event handler to catch FabTwain Events.
	...
	//
			...
			SELF:dwCurState := NO_TWAIN_STATE
	//
			SELF:lShowUI := TRUE
			SELF:hDib := NULL_PTR
			SELF:ptrDSM_Entry := NULL_PTR
...
// is for the User Interface or for our App.
			LOCAL msg	IS	_winMSG
	//
			WHILE ( ( SELF:dwCurState >= SOURCE_ENABLED )  .AND. GetMessage( @msg, NULL, 0, 0 ) )
				IF !( SELF:TwainDispatch( @msg ) )
					TranslateMessage( @msg )
					DispatchMessage( @msg)
				ENDIF
...
// then to this method where we get Image from DS then callback OnTwainDibAcquire()
	...
	//
		...
		// transfer failed for some reason or another
			...
			ENDIF
	//
			ASSERT( SELF:dwCurState >= TRANSFER_READY)
	// acknowledge & end transfer
			SELF:EndXfer()
	//
			ASSERT( (SELF:dwCurState == TRANSFER_READY)  .OR. (SELF:dwCurState == SOURCE_ENABLED) )
	//
...
//r	A Logical value indicating the success of the operation
	...
	//
			IF ( SELF:dwCurState != SOURCE_MANAGER_OPEN )
				RETURN FALSE
			ENDIF
...
//r A Logical value indicating the success of the operation
	...
	//
			IF SELF:__LoadSourceManager( )
				...
				ENDIF
			ENDIF
			RETURN ( SELF:dwCurState >= SOURCE_MANAGER_OPEN)
...
//d During the open, the DSM will assign an identifier to the App to track any further call
	//
		...
		// This ID will then be used by the Source Manger to recognise our App
				SELF:pAppID.Id := 0
		//
				SELF:pAppID.ProtocolMajor := TWON_PROTOCOLMAJOR
				SELF:pAppID.ProtocolMinor := TWON_PROTOCOLMINOR
	...
	//
		...
		//
			...
			// Something went wrong during transfer, cancel/abort
					SELF:DropToState( SOURCE_ENABLED )
					EXIT
				ENDIF
		//
				IF ( SELF:dwCurState != TRANSFER_READY )
